...
static ISMap constant_buffer_type_names = { { D3D_CT_CBUFFER, "ConstantBuffer" },
...
static USMap StorageTypeName = { { Invalid_type, "Invalid" }, { Float_type, "Float" },     { Half_type, "Half" },         { Int_type, "Int" },
                                 { UInt_type, "UInt" },       { Short_type, "Short" },     { UShort_type, "UShort" },     { Byte_type, "Byte" },
                                 { SByte_type, "SByte" },     { Norm16_type, "UShort" },   { Norm8_type, "SByte" },       { UNorm16_type, "UInt16" },
                                 { UNorm8_type, "Byte" },     { Typeless32_type, "UInt" }, { Typeless16_type, "UShort" }, { Typeless8_type, "Byte" } };

//////////////////////////////////////////////////////////////////////

static USMap shader_input_dimension_names = { { D3D_SRV_DIMENSION_UNKNOWN, "" },
...
InputType type_suffix[] = { "Invalid",    Invalid_type, "Float",     Float_type,   "Half",          Half_type,    "Int",           Int_type,    "UInt",
                            UInt_type,    "Short",      Short_type,  "UShort",     UShort_type,     "Int8",       SByte_type,      "SByte",     SByte_type,
                            "Byte",       UNorm8_type,  "UInt8",     Byte_type,    "Norm16",        Norm16_type,  "Norm8",         Norm8_type,  "UNorm16",
                            UNorm16_type, "UNorm8",     UNorm8_type, "Typeless32", Typeless32_type, "Typeless16", Typeless16_type, "Typeless8", Typeless8_type };
...
static DXGI_FORMAT formats[4][4] = { { DXGI_FORMAT_R32_TYPELESS, DXGI_FORMAT_R32_UINT, DXGI_FORMAT_R32_SINT, DXGI_FORMAT_R32_FLOAT },
                                     { DXGI_FORMAT_R32G32_TYPELESS, DXGI_FORMAT_R32G32_UINT, DXGI_FORMAT_R32G32_SINT, DXGI_FORMAT_R32G32_FLOAT },
                                     { DXGI_FORMAT_R32G32B32_TYPELESS, DXGI_FORMAT_R32G32B32_UINT, DXGI_FORMAT_R32G32B32_SINT, DXGI_FORMAT_R32G32B32_FLOAT },
                                     { DXGI_FORMAT_R32G32B32A32_TYPELESS, DXGI_FORMAT_R32G32B32A32_UINT, DXGI_FORMAT_R32G32B32A32_SINT, DXGI_FORMAT_R32G32B32A32_FLOAT } };
...
DXGI_FormatDescriptor DXGI_Lookup[] = { { DXGI_FORMAT_R32G32B32A32_FLOAT, "R32G32B32A32_FLOAT", 4, 32, Float_type },
...
void AddToBindingRun(vector<HLSLShader::BindingRun> &runs, uint bindPoint)
{
    ...
    if(startRun) {
        // start a new run
        runs.push_back({ (uint16)bindPoint, 1 });
    } else {
...
HLSLShader::HLSLShader(tstring const &filename) : mStartIndex(0), mName(String(filename)), mConstBuffers(0), mSamplers(0), mResources(0)
{
}
...
void HLSLShader::OutputConstructor()
{
    ...
    Indent();
    OutputLine(": %sShader(%d, %s, %d, %s, %d, %s, %s, %s, %s%s)", ShaderTypeName(), mConstantBufferBindings.size(), constBufferNames.c_str(), mSamplerBindings.size(),
               samplerNames.c_str(), mResourceBindings.size(), textureNames.c_str(), mResourceBindings.size() > 0 ? "textures" : "null",
               mSamplerBindings.size() > 0 ? "samplers" : "null", Format("%s_Bindings[ShaderType::%s]", mName.c_str(), mShaderTypeDesc.name).c_str(), vb_bindings.c_str());
...
{
    return (mShaderTypeDesc.type == ShaderType::Vertex) ? Format(", %s_InputElements, _countof(%s_InputElements)", Name().c_str(), Name().c_str()) : string();
}
...
HRESULT HLSLShader::CreateInputLayout()
{
    ...
    for(int i = 0; i < n; ++i) {
        ...
        --pe;
        if(mInputFields.size() > 0 && strcmp(d.SemanticName, pe->SemanticName) == 0 && d.Format == pe->Format && d.SemanticIndex == (pe->SemanticIndex + 1) &&
           d.InputSlot == pe->InputSlot && d.InputSlotClass == pe->InputSlotClass && d.InstanceDataStepRate == pe->InstanceDataStepRate) {
