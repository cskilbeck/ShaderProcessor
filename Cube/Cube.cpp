...
{
    float a = 1 / sqrtf(2);
    Vec4f p[] = { { -a, a, 0 },  { a, a, 0 },  { -a, -a, 0 }, { a, -a, 0 }, { 0, -a, a },  { 0, a, a },
                  { 0, -a, -a }, { 0, a, -a }, { a, 0, -a },  { a, 0, a },  { -a, 0, -a }, { -a, 0, a } };
...
int MyDXWindow::CreateOctahedron()
{
    ...
    Simple::InputVertex v[6] = { { { 0, 0, 1 }, 0xffffffff },  { { -f, f, 0 }, 0xffffffff },  { { f, f, 0 }, 0xffffffff },
                                 { { f, -f, 0 }, 0xffffffff }, { { -f, -f, 0 }, 0xffffffff }, { { 0, 0, -1 }, 0xffffffff } };
    DXR(octahedronVB.Create(6, v, StaticUsage));

    uint16 o[24] = { 0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 1, 5, 2, 1, 5, 3, 2, 5, 4, 3, 5, 1, 4 };
...
bool MyDXWindow::OnCreate()
{
    ...
    Resized += mWindowSized = [this](WindowSizedEvent const &s) { root.SetSize(FClientSize()); };

    root.Pressed += [this](UI::MouseEvent const &m) { mouseClicked = true; };

    // this should never happen...
    root.Released += [this](UI::MouseEvent const &m) { mouseClicked = false; };
    ...
    if(false) {
        ...
        auto remover = [](UI::ClickedEvent const &e) { e.mElement->Close(); };
        ...
        shape.MouseEntered += [&](UI::MouseEvent const &m) { shape.SetColor(0xff000000); };

        shape.MouseLeft += [&](UI::MouseEvent const &m) { shape.SetColor(0xffff0000); };
    }

    lightPos = Vec4(0, -1.5, 2, 0);
...
struct ClosestConvexResultCallback : public btCollisionWorld::ConvexResultCallback, Aligned16
{
    ClosestConvexResultCallback(btVector3 const &convexFromWorld, btVector3 const &convexToWorld)
        : m_convexFromWorld(convexFromWorld), m_convexToWorld(convexToWorld), m_hitCollisionObject(0)
    {
    }
...
void MyDXWindow::OnFrame()
{
    ...
    if(false) {
        ...
        Vec4f test[] = { Vec4(1, 1, 6), Vec4(-428.5751342773f, -475.6480102539f, 6.0997819901f) };
    ...
    if(false) {
        ...
        spriteSheet.Add(s, { sinf(time * 2) * 200 + 200, cosf(time * 1.5f) * 200 + 200 }, { sinf(time * 1.75f) * 0.1f + 0.2f, cosf(time * 1.9f) * 0.1f + 0.2f }, { 0.5f, 0.5f }, 0,
                        Color::White, xflip, yflip);
