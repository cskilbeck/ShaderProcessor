...
void DrawList::SetViewport(Rect2D const &rect)
{
    ViewportItem *vi = Add<ViewportItem>();
    vi->mViewport = rect;
}
...
void DrawList::Execute()
{
    ...
    while(t < end) {
        ...
        case it_Viewport: {
            ViewportItem *i = (ViewportItem *)t;
            D3D11_VIEWPORT v;
            v.TopLeftX = (float)i->mViewport.left;
            v.TopLeftY = (float)i->mViewport.top;
            v.Width = (float)i->mViewport.Width();
            v.Height = (float)i->mViewport.Height();
            v.MaxDepth = 1;
            v.MinDepth = 0;
            mContext->RSSetViewports(1, &v);
            t += sizeof(ViewportItem);
        } break;
