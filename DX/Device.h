...
{
    explicit BackBufferCount(uint count) : mCount(count)
    {
    }
...
struct DisplayMode : list_node
{
    ...
    {
        static char const *scalingModeNames[] = { "Unspecified", "Centered", "Stretched" };
...
struct D3DDevice
{
    D3DDevice(CreateSwapChainOption createSwapChainOption = WithSwapChain, BackBufferCount backBufferCount = BackBufferCount(2),
              DepthBufferOption depthBufferOption = DepthBufferDisabled, FullScreenOption fullScreenOption = FullScreen, DXGI_SWAP_EFFECT swap_effect = DXGI_SWAP_EFFECT_DISCARD)
        : mWindow(null)
        , mWidth(0)
        , mHeight(0)
...
//#define D3D_DEBUG
        ...
        if(mCreateSwapChainOption == WithSwapChain) {
            ...
            swapChainDesc.SwapEffect = mSwapEffect;
